...
import { Pencil, Search, Trash2, Users2, XIcon } from 'lucide-react'
...
export default function Users() {
    ...
    const fetchUsers = (page) => {
        setLoading(true)
        axios.get(`${import.meta.env.VITE_HOST}/admin/users/all?page=${page}`)
            .then(res => {
    ...
    const fetchSearchedUsers = (text, page) => {
        setLoading(true)
        axios.get(`${import.meta.env.VITE_HOST}/admin/users/search?searchText=${text}&page=${page}`)
            .then(res => {
                if (res.status === 200) {
    ...
    const handleUpdateUser = async () => {
        ...
        setLoading(true);
        try {
            const res = await axios.put(`${import.meta.env.VITE_HOST}/admin/users/update-role/${selectedUser._id}`, { role: newRole });
            if (res.status === 202) {
                setShowUpdateModal(false);
                setSelectedUser(null);
                window.toastify("User role updated!", "success");
    ...
    const handleBanUser = async (user) => {
        setLoading(true);
        try {
            const res = await axios.put(`${import.meta.env.VITE_HOST}/admin/users/update-status/${user._id}`, { status: user.status });
            if (res.status === 202) {
    ...
    const handleDeleteUser = async (id) => {
        ...
        setLoading(true);
        try {
            const res = await axios.delete(`${import.meta.env.VITE_HOST}/admin/users/delete/${id}`);
            if (res.status === 203) {
    ...
    return (
        <div className='admin-container'>
            <h1 className='flex items-center gap-1 text-xl'><Users2 size={20} /> Users</h1>
            <p className='w-fit px-2 py-0.5 text-sm text-[var(--secondary)] bg-[var(--light)]/50 rounded mt-1'>See registered users here and manage them</p>
                ...
                <table className="w-full text-xs text-left rtl:text-right text-gray-600">
                    <thead className="text-[10px] text-[var(--secondary)] bg-white uppercase border-b border-gray-200">
                        <tr>
                            ...
                            <th scope="col" className="font-bold px-6 py-4">Banned</th>
                                    ...
                                    users.map((user, i) => (
                                        <tr className="bg-white border-b border-gray-200">
                                            ...
                                            <td className="px-6 py-4 capitalize">
                                                <span className={`text-[10px] px-2 py-1 font-bold rounded ${user.role === 'admin' ? 'text-[#0ec520] bg-[#d1ffd5]' : 'text-[#4b8fd9] bg-[#e4f1ff]'}`}>
                                                    {user.role}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className={`relative w-10 h-5 cursor-pointer rounded-full transition-all duration-200 ease-out ${user.status === "banned" ? "bg-blue-400" : "bg-gray-200"}`}
                                                    onClick={() => handleBanUser(user)}
                                                >
                                                    <div className={`absolute left-0.5 top-1/2 -translate-y-1/2 w-4 h-4 bg-white rounded-full transition-all duration-200 ease-out ${user.status === "banned" ? "translate-x-[calc(100%+4px)]" : "translate-x-0"}`}></div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
